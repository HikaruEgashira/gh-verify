...
## 削除条件 (全達成: v0.4.0)
- [x] `gh-verify-core` から GitHub 固有語彙が消えている
- [x] CLI の評価入口が `RuleContext` ではなく `EvidenceBundle` になっている
- [x] 互換コードが CLI 境界から削除されている
- [x] 旧 rule 実装が新主系の wrapper すら持たず削除されている
- [x] CI と Action が profile ベースの出力だけで成立している
- [x] `cargo test --workspace` が control/evidence 主系のみで green

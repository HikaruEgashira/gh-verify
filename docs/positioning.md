...
- **検証ロジックはlibverifyに集約**: gh-verifyはlibverifyのGitHub向けフロントエンド。コントロール・ポリシー・出力フォーマッタはlibverifyが提供し、gh-verifyはGitHub APIアダプタとCLIのみを担う

## エコシステム

| リポジトリ | 役割 |
|---|---|
| [libverify](https://github.com/HikaruEgashira/libverify) | プラットフォーム非依存の検証エンジン (コントロール、ポリシー、出力) |
| [gh-verify](https://github.com/HikaruEgashira/gh-verify) | GitHub向けフロントエンド (GitHub API + gh CLI拡張) |
| [atlassian-verify](https://github.com/HikaruEgashira/atlassian-verify) | Atlassian向けフロントエンド (Bitbucket API + Jira連携) |
...
| 機能 | 理由 |
|------|------|
| GitHub外ツール連携 (Jira, Confluence等) | 1 tool = 1 platform。atlassian-verifyが担当 |
